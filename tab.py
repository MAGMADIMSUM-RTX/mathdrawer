...
line2, = sub.plot(df2['V'], df2['I'], marker='^', markersize=4, linewidth=2,
...
line2, = sub2.plot(np.log(df2['Iin']), df2['V'], marker='^', markersize=4, linewidth=2,
                   linestyle=':',  color='black')
...
line1, = sub3.plot(df['Iin'], df['Isc'],  marker='o', markersize=4, linewidth=2,
                   linestyle='-',  color='red')
df2 = pd.read_excel('./dat.xlsx', sheet_name='Sheet4')
line2, = sub3.plot(df2['Iin'], df2['Isc'], marker='^', markersize=4, linewidth=2,
                   linestyle=':',  color='black')
...
line2, = sub4.plot(df2['R'], df2['I']**2*df['R']/1000, marker='^', markersize=4, linewidth=2,
                   linestyle=':',  color='black')
sub4.legend(handles=[line1, line2], labels=[
    '单晶硅', '非晶硅'], loc='best')
sub4.grid(which='major')
sub4.set_ylabel(r'$I^2R$(W)', fontsize=12, rotation=0, loc='top')
